...
- The entire lifecycle is finished. Delegate to the `bmad-worker` subagent with `reset-progress=true` and the `template` parameter to start a fresh lifecycle:

> **Delegate to subagent `bmad-worker`:**
> `"reset-progress=true template=<template> module=<module>"`
...
### 2.1 — Delegate to bmad-worker (auto-discover)

> **Delegate to subagent `bmad-worker`:**
> Build the prompt from parameters. For a standard auto-discover call:
...
> **Delegate to subagent `pr-create`:**
> If `force` parameter is set, include `"force=true"` in the prompt. Otherwise use a standard prompt describing that this PR contains BMAD workflow artifacts.
