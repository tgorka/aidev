---
description: >
  BMAD workflow step executor with auto-discovery. Automatically determines the
  next workflow task when invoked without explicit instructions. Tracks lifecycle
  progress per worktree/project via named progress files. Supports lifecycle
  templates (full, feature, bugfix, brownfield, minimal), approval gates for key
  deliverables, and worktree-aware operation. Use for all BMAD workflow delegation.
mode: subagent
---
